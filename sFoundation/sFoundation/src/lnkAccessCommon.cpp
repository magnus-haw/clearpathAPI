...
netStateInfo::netStateInfo(nodeulong ringCmdsMax,
    ...
    LastEvent = NODES_UNKNOWN;              // We start from unknown for initialization
    pAutoDiscover = NULL;
    // Offline tickler
    ...
    pTrkFixedList =
        (respTrackInfo **)calloc(ringCmdsMax, sizeof(respTrackInfo *));
    pTrkList = (respTrackInfo **)calloc(ringCmdsMax, sizeof(respTrackInfo *));
...
//
//  SYNOPSIS:
unsigned mnNetInvRecords::logSend(packetbuf *cmd, cnErrCode theErr,
                                  double timeStamp) {
...
double mnNetInvRecords::logReceive(
    ...
    if (theErr != MN_OK && pNCS->cNum == 1) {
        _RPT4(_CRT_WARN,
              "%.1f RX Error(%d): 0x%x, thread=" THREAD_RADIX_LONG "\n",
...
void netStateInfo::removeThisDBitem(
    ...
    // No node list
    if ((pRespArea == NULL) || (pRespInfo == NULL) ||
        pRespArea->head == NULL) {
        return;
    }
    ...
    if (pRespInfo->evtRespWait.isOK()) {
        ...
        _RPT2(_CRT_WARN, "%.1f removeDB wait event set hndl=0x%0lx\n",
              infcCoreTime(),
              reinterpret_cast<long>(pRespInfo->evtRespWait.GetHandle()));
#endif
...
#define RD_TIME_TRC 0
    ...
    while (!((m_pTermFlag != NULL) && *m_pTermFlag)
           && m_state != READ_SHUTDOWN) {
#if TRACE_RD_THRD
        if (m_state == READ_HALTED) {
            _RPT2(_CRT_WARN,
                  "%.1f readThread(%d) halted and waiting for restart\n",
...
#define RD_THREAD_PREMPTIVE_WAIT 100
                ...
                if (theNet.PortIsOpen()) {
                    ...
                    waitOK =
                        pNCS->ReadCommEvent.WaitFor(RD_THREAD_PREMPTIVE_WAIT);
                }
                else {
                ...
                if (doTheRead && !((m_pTermFlag != NULL) && *m_pTermFlag)) {
                    ...
                    if (theErr == MN_OK) {
                        ...
                        if (readBuf.Fld.Src == MN_SRC_HOST) {
                            netErrGeneric *pGenErr =
                                (netErrGeneric *)&readBuf.Byte.Buffer[2];
                        ...
                        if (readBuf.Byte.BufferSize == 0) {
                            ...
                            if (readBuf.Fld.Src == MN_SRC_HOST) {
                                ...
                                netErrGeneric *pErrPkt =
                                    (netErrGeneric *)&readBuf.Byte.Buffer[RESP_LOC];
                                bool isNetReject =
                                    ((unsigned)readBuf.Fld.PktType == MN_PKT_TYPE_ERROR)
                                ...
                                if (isNetReject) {
                                    ...
                                    _RPT4(_CRT_WARN, "%8d%8d%8d%8d",
                                          ...
                                          pLog->AppNetOverrunCtr);
                                    _RPT4(_CRT_WARN, "\t%8d%8d%8d%8d\n",
                                          pLog->DiagNetFragPktCtr,
                                          pLog->DiagNetBadChksumCtr,
                                          pLog->DiagNetStrayCtr,
                                          pLog->DiagNetOverrunCtr);
#endif
                                ...
                                // Expecting response?
                                    ...
                                    pFillInfo->stats.pacedTime =
                                        infcCoreTime() - pFillInfo->funcStartAt;
                                    // Record receive time - send time and packet received
                                    pFillInfo->stats.execTime
                                        = theNet.logReceive(&readBuf, theErr,
                                                            pFillInfo, rxTime);
                                    // Signal performance outcomes
                                    if (userCmdCompleteFunc != NULL &&
                                        theNet.TraceActive) {
                                        (*userCmdCompleteFunc)(pNCS->cNum,
                                                               &pFillInfo->stats);
                                    }
                                    ...
                                    if (!pNCS->SelfDestruct) {
                                        ...
                                        infcCopyPktToPkt18(&errInfo.response,
                                                           &readBuf);
                                        switch (readBuf.Fld.PktType) {
                                            case MN_PKT_TYPE_ERROR:
                                                theErr =
                                                    coreGenErrCode(pNCS->cNum,
                                                                   &readBuf,
                                                                   (nodeaddr)errInfo.node);
                                                theNet.logReceive(&readBuf,
                                                                  theErr, NULL,
                                                                  rxTime);
                                                // Release cmd lock on database
                                                ...
                                                _RPT2(_CRT_WARN,
                                                      "readThread(%d) MN_PKT_TYPE_ERROR detected by node %d\n",
                                            ...
                                            default:
                                                ...
                                                _RPT2(_CRT_WARN,
                                                      "readThread(%d): unsolicited packet response: node=%d\n",
                                                      pNCS->cNum, respAddr);
                                ...
                                // Log the reception
                                theNet.logReceive(&readBuf, theErr, NULL,
                                                  rxTime);
                                ...
                                processNodeInitiatedPkt(pNCS->cNum, respAddr,
                                                        pNCS, readBuf);
                            } // src=MN_SRC_MODE
                        } // if (bufferSize==0)
                    }
                        ...
                        if (theNet.OpenState == FLASHING) {
                            ...
                            _RPT3(_CRT_WARN,
                                  "%.1f readThread(%d): read failed code=0x%x\n",
...
void readPktThread::Start() {
    ...
    switch (m_state) {
        ...
        default:
            ...
            ReadLock.Unlock();
            _RPT2(_CRT_ASSERT,
                  "readThreadStart(%d) unknown read state handling %d\n",
                  pNCS->cNum, m_state);
            break;
    }
...
void autoDiscoverStart(infcErrInfo *pErrInfo) {
    ...
    if (theNet.Initializing || InitializingGlobal) {
        //  if (theNet.Initializing) {
        _RPT3(_CRT_WARN,
              "%.1f autoDiscoverStart(%d) ignored: initializing, global=%d\n",
    ...
    if ((pNCS->pAutoDiscover && pNCS->pAutoDiscover->IsRunning())
        ...
        _RPT2(_CRT_WARN,
              "%.1f autoDiscoverStart(%d): running or already shutdown\n",
              infcCoreTime(), cNum);
        return;
    }
...
autoDiscoverThread::~autoDiscoverThread() {
    ...
    double sTime = infcCoreTime();
    _RPT2(_CRT_WARN,
          "%.1f autoDiscoverThread destructing, id=" THREAD_RADIX "\n",
...
int autoDiscoverThread::Run(void *context) {
    ...
    _RPT3(_CRT_WARN,
          "%.1f autoDiscoverThread(%d): starting, id=" THREAD_RADIX "\n",
    ...
    while ((m_pTermFlag == NULL) || (!(*m_pTermFlag))) {
#if TRACE_TICKLER
        _RPT3(_CRT_WARN,
              "%.1f autoDiscoverThread(%d): parking, id=" THREAD_RADIX "\n",
              infcCoreTime(), cNum, UIthreadID());
#endif
        ...
        // If transitioning to online, clear the initializing state
        if (m_goneOnline && forcedOffline) {
            forcedOffline = false;
...
restartOnlineSearch:
        while (((WaitingForNet() && !m_goneOnline) || forcedOffline)
               && !m_exitDiscovery) {
...
#if TRACE_HIGH_LEVEL|TRACE_TICKLER
            _RPT5(_CRT_WARN,
                  "%.1f in autoDiscoverThread(%d) restarted #%d, forced=%d, "
...
#if TRACE_TICKLER
                _RPT2(_CRT_WARN,
                      "%.1f in autoDiscoverThread(%d) post-flush cancel\n",
                      infcCoreTime(), cNum);
#endif
                break;
            }
            ...
            if (IsNetClassErr(theNet.OfflineRootErr)) {
#if TRACE_HIGH_LEVEL|TRACE_TICKLER
                _RPT4(_CRT_WARN,
                      "%.1f in autoDiscoverThread(%d) net class err 0x%x,"
                      "thread ID=" THREAD_RADIX "\n",
...
#if TRACE_BREAK
                _RPT3(_CRT_WARN,
                      "%.1f in autoDiscoverThread(%d) "
                      "calling infcResetNetRate,"
                      "thread ID=" THREAD_RADIX "\n",
                      infcCoreTime(), cNum, UIthreadID());
#endif
                ...
                if (WaitingForNet() && !hasConnectivity) {
                    ...
                    if (SysInventory[cNum].PhysPortSpecifier.PortType
                        == MERIDIAN_CONMOD) {
#if TRACE_HIGH_LEVEL||TRACE_TICKLER
...
#endif
                        theErr = infcBrokenRingDiag(cNum, hasConnectivity,
                                                    nNodes);
...
#if TRACE_HIGH_LEVEL||TRACE_TICKLER
                    _RPT3(_CRT_WARN,
                          "%.1f autoDiscoverThread(%d) probe for "
                          "offline nodes err=%x\n",
                          infcCoreTime(), cNum, theErr);
#endif
                    infcErrInfo errRep;
                ...
                theErr = infcChangeLinkSpeed(cNum,
                    SysInventory[cNum].PhysPortSpecifier.PortRate);
                if (theErr != MN_OK) {
                    continue;
                }
...
#if TRACE_HIGH_LEVEL|TRACE_TICKLER
                _RPT3(_CRT_WARN,
                      "%.1f autoDiscoverThread(%d) errors unhandled, "
...
#if TRACE_TICKLER
        _RPT5(_CRT_WARN,
              "%.1f autoDiscoverThread(%d) going online, initsema=%d"
        ...
        if (!shouldTerm) {
            if (m_goneOnline) {
...
#if TRACE_HIGH_LEVEL||TRACE_TICKLER
                    _RPT4(_CRT_WARN,
                          "%.1f autoDiscoverThread(%d) node count change"
                          ", now=%d, was=%d\n",
                          infcCoreTime(), cNum,
                          SysInventory[cNum].InventoryNow.NumOfNodes,
                ...
                nodeaddr nAddr;
                for (nAddr = 0;
                     nAddr < SysInventory[cNum].InventoryNow.NumOfNodes;
                     nAddr++) {
                    devID_t dID;
        ...
        if (shouldTerm) {
            ...
            _RPT4(_CRT_WARN,
                  "%.1f autoDiscoverThread(%d): going back online=%d, id="
                  THREAD_RADIX "\n",
                  infcCoreTime(), cNum, m_goneOnline, UIthreadID());
            _RPT5(_CRT_WARN,
                  "%.1f autoDiscoverThread(%d) initializing (non-term)=%d"
...
#if TRACE_LOW_PRINT||TRACE_TICKLER
        _RPT2(_CRT_WARN,
              "%.1f autoDiscoverThread::Start from parked: init cnt=%d\n",
              infcCoreTime(), SysInventory[pNCS->cNum].Initializing);
#endif
...
MN_EXPORT cnErrCode MN_DECL infcResetNetRate(
    ...
    switch (serErr) {
        ...
        case CSerial::API_ERROR_PORT_SETUP:
            _RPT2(_CRT_WARN,
                  "%.1f infcResetNetRate(%d) default net rate not supported!\n",
                  infcCoreTime(), cNum);
            return (MN_ERR_BAUD_NOT_SUPPORTED);
        default:
            _RPT3(_CRT_WARN,
                  "%.1f infcResetNetRate(%d) port unavailable! Err=0x%x\n",
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (serErr) {
            _RPT3(_CRT_WARN,
                  "%.1f infcResetNetRate(%d) sending break failed, err=0x%x\n",
                  infcCoreTime(), cNum, serErr);
            return (MN_ERR_PORT_PROBLEM);
        }
        ...
        if (currentErrors.BREAKcnt == 0) {
            _RPT2(_CRT_WARN,
                  "%.1f infcResetNetRate(%d) no BREAK was detected\n",
                  infcCoreTime(), cNum);
    ...
    if (theErr != MN_OK) {
        ...
        switch (winErr) {
...
#if TRACE_BAUD
                _RPT3(_CRT_WARN,
                      "%.1f infcChangeLinkSpeed(%d) port now @ %d\n",
                      infcCoreTime(), cNum, newRate);
#endif
            ...
            case CSerial::API_ERROR_PORT_SETUP:
                ...
                _RPT4(_CRT_WARN,
                      "%.1f infcChangeLinkSpeed(%d) port doesn't support %d, "
            ...
            default:
                ...
                _RPT2(_CRT_WARN,
                      "%.1f infcChangeLinkSpeed(%d), port unavailable\n",
                      infcCoreTime(), cNum);
                break;
        }
    }
...
MN_EXPORT cnErrCode MN_DECL infcRunCommand(
    ...
    if (!sleepOK) {
        ...
        infcErrInfo semaErr;
        _RPT1(_CRT_WARN,
              "infcRunCommand(%d): pace semaphore release err, deadlocked?\n",
              cNum);
    ...
    *pNCS->pTrkTail-- = NULL;
    ...
    if (theErr == MN_OK) {
        ...
        pRespInfo->sendSerNum = theNet.logSend(theCommand, theErr,
                                               pRespInfo->cmdStartAt);
        ...
        if (!waitOK) {
            ...
            if (SysInventory[cNum].TraceActive) {
                ...
                theNet.logReceive(&nullPkt, MN_ERR_RESP_TIMEOUT,
                                  pRespInfo, infcCoreTime());
            }
            if (SysInventory[cNum].OpenState == OPENED_ONLINE) {
...
MN_EXPORT cnErrCode MN_DECL infcStartController(
    ...
    if (!theNet.txTraces) {
        theNet.txTraces = new txTraceBuf[SEND_DEPTH];
    }
    if (!theNet.rxTraces) {
        theNet.rxTraces = new rxTraceBuf[RECV_DEPTH];
    }

    // Allow only one starter at a time
    SysInventory[cNum].onlineOfflineLock.Lock();
    ...
    if (!pNCS) {
        pNCS = SysInventory[cNum].pNCS =
            new netStateInfo(SysInventory[cNum].NumCmdsInRing, cNum);
#if TRACE_SIZES
        ...
        // Did it work out?
        if (!pNCS) {
            SysInventory[cNum].onlineOfflineLock.Unlock();                          // Unlock
            return (MN_ERR_MEM_LOW);
        }
        // Initialize the offline state
        // This is done outside the netStateInfo constructor so that
        // a transition is seen and the userNetStateFunc is called
        infcFireNetEvent(cNum, NODES_OFFLINE);
    }
    ...
    if (!pNCS->pSerialPort
        ...
        // Unlock
        SysInventory[cNum].onlineOfflineLock.Unlock();
        return (MN_ERR_PORT_PROBLEM);
    }
    ...
    if (theErr == MN_OK) {
        ...
        if (theErr == MN_OK) {
            theErr = infcBrakeSet(cNum, 1, SysInventory[cNum].brake1Saved,
                true);
        }
        if (theErr != MN_OK) {
            SysInventory[cNum].onlineOfflineLock.Unlock();                          // Unlock
...
#if TRACE_BREAK
    _RPT3(_CRT_WARN,
        "%.1f in infcStartController(%d) calling infcResetNetRate,"
        "thread ID=" THREAD_RADIX_LONG "\n",
        infcCoreTime(), cNum, infcThreadID());
#endif
...
{
    return infcStopControllerEx(cNum, NODES_OFFLINE);
}
cnErrCode MN_DECL infcStopControllerEx(
    netaddr cNum,               // Index of controller desired
    enum NetworkChanges state)  // Next NetEvent state
{
    netStateInfo *pNCS;
    cnErrCode theErr = MN_OK;
#if TRACE_START_STOP
    ...
    SysInventory[cNum].onlineOfflineLock.Lock();                          // Lock
    ...
    if (pNCS->pSerialPort->IsOpen()) {
        ...
        // Send the requested next NetEvent state
...
#define LCL_INIT_PR 0
    ...
    if (initializing) {
        ...
        _RPT4(_CRT_WARN,
              "%.1f infcSetInitializeMode(%d): Initializing #%d, err=0x%x\n",
...
#if (TRACE_CALLBACK&&TRACE_LOW_LEVEL) || LCL_INIT_PR
            _RPT3(_CRT_WARN,
                  "%.1f infcSetInitializeMode(%d): "
                  "Initialization complete, err=0x%x\n",
            ...
            if (state == NODES_DRIVER_ONLINE) {
                for (unsigned i = 0; i < theNet.InventoryNow.NumOfNodes; i++) {
                    ...
                    if (theErr != MN_OK) {
                        _RPT3(_CRT_WARN,
                              "%.1f infcSetInitializeMode(%d): "
                              "failed to clear net stats, err=0x%x\n",
                              infcCoreTime(), cNum, theErr);
                        lastErr = theErr;
                        break;
                    }
                ...
                if (theNet.PhysPortSpecifier.PortType == CPM_COMHUB
                    ...
                    for (size_t i = 0; i < mnNetInvRecords::MAX_BRAKES; i++) {
                        nodeaddr theAddr =
                            NODE_ADDR(theNet.autoBrake[i].relatedNode);
                        ...
                        if (theNet.autoBrake[i].relatedNode == MN_UNSET_ADDR
                            ...
                            theNet.autoBrake[i].relatedNode =
                                MULTI_ADDR(cNum, i);
                        }
                    }
                    ...
                    for (size_t i = 0;
                         i < theNet.InventoryNow.mnCpScInfo.count; i++) {
                        multiaddr theAddr =
                            theNet.InventoryNow.mnCpScInfo.node[i];
                        ShutdownInfo &sInfo =
                            theNet.GroupShutdownInfo[NODE_ADDR(theAddr)];
                        infcShutdownInfoSet(theAddr, &sInfo);
                        // Restore the driver attention masks
                        netSetDrvrAttnMask(theAddr,
                            &theNet.drvrAttnMask[NODE_ADDR(theAddr)]);
                    }
...
#if (TRACE_CALLBACK&&TRACE_LOW_LEVEL) || LCL_INIT_PR
            _RPT4(_CRT_WARN,
                  "%.1f infcSetInitializeMode: heading online "
                  "#%d offline=%d, err=0x%x\n",
...
MN_EXPORT void MN_DECL infcSetTraceEnable(
    netaddr cNum,
    nodebool state) {
    SysInventory[cNum].TraceActiveReq = state;
    SysInventory[cNum].TraceActive = state;
}
//                                                                             *
//******************************************************************************
...
void infcCopyPktToPkt18(packetbuf18 *dest, packetbuf *src) {
    ...
    memcpy(&dest->buffer[0], &src->Byte.Buffer[0], dest->bufferSize);
}
//                                                                            *
//*****************************************************************************
...
inline void processNodeInitiatedPkt(netaddr cNum, nodeaddr respAddr,
    ...
    mnNetDiagStats *pStats =
        (mnNetDiagStats *)(&readBuf.Byte.Buffer[CMD_LOC + 1]);
    ...
    switch (readBuf.Fld.PktType) {
            ...
            switch (readBuf.Byte.Buffer[CMD_LOC]) {
                case MN_CTL_EXT_NET_DIAG_INFO:
                    ...
                    if ((readBuf.Fld.PktLen - 1) / 2U
                        <= (unsigned)(&pStats->NetVoltsLowCtr 
                                      - (Uint16 *)pStats)) {
                    ...
                    *pLog = *pStats;
                    _RPT3(_CRT_WARN,
                          "%.1f readThread(%d) node %d reported err: \n",
                          infcCoreTime(), cNum, readBuf.Fld.Addr);
                    DUMP_PKT(cNum, (char *)"...", &readBuf, true);
#ifdef _DEBUG
                    _RPT2(_CRT_WARN,
                          "Net Diag Counters:\tApp Net\t\t\t\tDiag Net "
                          "reporting node %d, net %d\n",
                          readBuf.Fld.Addr, cNum);
                    _RPT0(_CRT_WARN,
                          "\tfrags\tchks\tstray\tover\t\tfrags\tchks\tstray"
                           "\tover\tlow V\n");
                    _RPT4(_CRT_WARN, "%8d%8d%8d%8d",
                ...
                case MN_CTL_EXT_NET_CHECK:
                    _RPT2(_CRT_WARN, "readThread(%d) detected net integrity "
                          "check request from node %d\n",
                          cNum, readBuf.Fld.Addr);
                    break;
                default:
                    _RPT2(_CRT_WARN, "readThread(%d) detected unhandled node "
                          "generated extended high packet %d\n",
        ...
        case MN_PKT_TYPE_ATTN_IRQ:
            attn.AttentionReg.attnBits =
                *((Uint32 *)&readBuf.Byte.Buffer[RESP_LOC]);
            // Cleanup the attention bits as required by device
            attn.AttentionReg.attnBits &=
                SysInventory[cNum].attnCleanupMask[readBuf.Fld.Addr];
        ...
        case MN_PKT_TYPE_EXTEND_LOW:
            switch (readBuf.Byte.Buffer[CMD_LOC]) {
                ...
                case MN_CTL_EXT_DATA_ACQ:   // Data return originated from the node and is a dataAcq point
                    ...
                    dataAcqPacket =
                        (dacqPacket *) & (readBuf.Byte.Buffer[RESP_LOC + 1]);
                    ...
                    switch (pNCS->DataAcqMode[respAddr]) {
                        case SCP_DRATE:
                        case SCP_DRATEx2:
                            pNCS->DataAcq[respAddr].SampleCount += 2;
                            break;
                        case SCP_DUAL_CH:
                            pNCS->DataAcq[respAddr].SampleCount++;
                            break;
                        default:
                            break;
                    }
                    ...
                    if (readBuf.Byte.BufferSize == 9) {
                        dataAcqPt[0].TraceValue[1] =
                            (float)((dataAcqPacket->Fld.Trigger0
                                     << (INPUT_BITS + 1))
                            | (dataAcqPacket->Fld.Inputs0 << 1)
                            | dataAcqPacket->Fld.Output0);
                        dataAcqPt[1].TraceValue[1] =
                            (float)((dataAcqPacket->Fld.Trigger1
                                     << (INPUT_BITS + 1))
                            | (dataAcqPacket->Fld.Inputs1 << 1)
                            | dataAcqPacket->Fld.Output1);
                    }

                    // Calculate fractional (+/-1) second point if in dual trace mode
                    // NOTE: this is placed in TraceValue[2] for backwards compatibility mode
                    dataAcqPt[0].TraceValue[2]
                        = (float)dataAcqPacket->Fld.T0p1
                          / (1 << (P1_BITS - 1));
                    ...
                    if (pNCS->DataAcqInit[respAddr]
                        ...
                        _RPT4(_CRT_WARN,
                              "%.1f data acq seq err@node %d: %d->%d\n",
                              infcCoreTime(), respAddr,
                              pNCS->DataAcq[respAddr].SeqCheck,
                              dataAcqPt[0].Sequence);
                        infcFireErrCallback(&errInfo);
#if TRACE_DACQ_SEQ
                    ...
                    // Queue the data to the host
                    pNCS->DataAcq[respAddr].Points.push(dataAcqPt[0]);
                    if (pNCS->DataAcqMode[respAddr] == SCP_DRATE ||
                            pNCS->DataAcqMode[respAddr] == SCP_DRATEx2) {
                        // Add the second point to the queue
                ...
                case MN_CTL_EXT_PARAM_CHANGED:  //A parameter on the node has changed
                    ...
                    break;
                default:
                    _RPT2(_CRT_WARN, "readThread(%d) detected unhandled "
                          "MN_PKT_TYPE_EXTEND_LOW packet type=%d\n",
                          cNum, readBuf.Byte.Buffer[CMD_LOC]);
                    break;
            }
            break;
...
inline void processAttention(
    ...
    _RPT3(_CRT_WARN, "%.1f attn(%d)=>0x%lx\n", infcCoreTime(),
          theAttn.MultiAddr, theAttn.AttentionReg.attnBits);
#endif
...
#if TRACE_ATTN
        _RPT5(_CRT_WARN,
              "%.1f attn(%d) going disabled=%d, disabled=%d, enabled=%d\n",
              infcCoreTime(), theAttn.MultiAddr,
              theAttn.AttentionReg.cpm.GoingDisabled,
              theAttn.AttentionReg.cpm.Disabled,
              theAttn.AttentionReg.cpm.Enabled);
#endif
        for (Uint32 i = 0; i < mnNetInvRecords::MAX_BRAKES; i++) {
            mnNetInvRecords::autoBrakeInfo &brk = theNet.autoBrake[i];
            if (brk.enabled && theAttn.MultiAddr == brk.relatedNode
    ...
    theAttn.AttentionReg.attnBits &=
        theNet.attnMask[NODE_ADDR(theAttn.MultiAddr)].attnBits;
    // Tell the user?
        ...
        if (SysInventory[cNum].pPortCls->Adv.Attn.Enabled()) {
            if (SysInventory[cNum].pPortCls->Adv.Attn.HasAttnHandler()) {
                ...
                _RPT2(_CRT_WARN, "%.1f attn(%d) invoking user callback\n",
                      infcCoreTime(), theAttn.MultiAddr);
#endif
                SysInventory[cNum].pPortCls->
                    Adv.Attn.InvokeAttnHandler(theAttn);
            }
            // Try using the by-node signaler
            for (int iNode = 0;
                 iNode < SysInventory[cNum].pPortCls->NodeCount(); iNode++) {
                if (SysInventory[cNum].pPortCls->
                        Nodes(iNode).Info.Ex.Addr() == theAttn.MultiAddr) {
                    SysInventory[cNum].pPortCls->
                        Nodes(iNode).Adv.Attn.SignalAttn(theAttn.AttentionReg);
                }
            }
                ...
                if ((((initCntIn == 0) && InitializingGlobal == 0)
...
#if TRACE_CALLBACK
                        _RPT4(_CRT_WARN, "%.1f infcFireErrCallback(%d) "
                              "ignoring dup err: err=0x%x, node=%d\n",
                              infcCoreTime(), (int)cpyErr.cNum,
                              cpyErr.errCode, (int)cpyErr.node);
#endif
                        break;
                    }
#if TRACE_CALLBACK
                    _RPT4(_CRT_WARN,
                          "%.1f infcFireErrCallback(%d): err=0x%x, node=%d\n",
                          infcCoreTime(), (int)cpyErr.cNum, cpyErr.errCode,
                          (int)cpyErr.node);
#endif
                    ...
                    if (!pNCS->SelfDestruct) {
                        ...
                        if (cpyErr.errCode >= MN_ERR_CMD_ERR_BASE
                            ...
                            _RPT4(_CRT_WARN,
                                  "%.1f infcFireErrCallback(net rejected "
                                  "type): err=0x%x, node=%d, net=%d\n",
                                  infcCoreTime(), pErrInfo->errCode,
                                  (int)cpyErr.node, cpyErr.cNum);
#endif
                        }
                        else {
                            ...
                            _RPT4(_CRT_WARN,
                                  "%.1f infcFireErrCallback(other type): "
                                  "err=0x%x, node=%d, net=%d\n",
                                  infcCoreTime(), pErrInfo->errCode,
                                  (int)cpyErr.node, (int)cpyErr.cNum);
#endif
                            // Log lead-up if possible
                            infcTraceDumpNext(cpyErr.cNum);
                            // If we allow diagnostics and this is not a result of
                            // the diagnostics, run them and start auto-discover to
                            // watch for network returning to life.
                            if (IsNetBreakErr(cpyErr.errCode)) {
...
void infcFireNetEvent(netaddr cNum, enum NetworkChanges state) {
    ...
    if (state != pNCS->LastEvent) {
        ...
        if (userNetStateFunc != NULL && !InitializingGlobal) {
            (*userNetStateFunc)(cNum,
                SysInventory[cNum].OpenState == OPENED_ONLINE);
        }
    }
...
void infcFireParamCallback(paramChangeFunc func, const mnParamChg *pChgItem) {
    ...
    if (!pNCS->isRecoveryThread() && !pNCS->SelfDestruct) {
        ...
        if (mnRef[copyChg.net] != NULL) {
            ...
            long inCurse =
                InterlockedIncrement(&ncs[copyChg.net].ChangeRecurse);
            // Marshall the net
...
MN_EXPORT cnErrCode MN_DECL infcGetNetErrorStats(
    ...
    pResult->AppNetBadChksumCtr = -1;
    pResult->AppNetFragPktCtr = -1;
    pResult->AppNetOverrunCtr = -1;
    pResult->AppNetStrayCtr = -1;
    pResult->DiagNetBadChksumCtr = -1;
    pResult->DiagNetFragPktCtr = -1;
    pResult->DiagNetOverrunCtr = -1;
    pResult->DiagNetStrayCtr = -1;
    pResult->NetVoltsLowCtr = -1;
    // Grab all the data from the node
    theErr = coreBaseGetParameterInt(cNum, nodeAddr, MN_P_NETERR_APP_CHKSUM,
                                     &stat);
    if (theErr == MN_OK) {
    ...
    theErr = coreBaseGetParameterInt(cNum, nodeAddr, MN_P_NETERR_APP_FRAG,
                                     &stat);
    if (theErr == MN_OK) {
    ...
    theErr = coreBaseGetParameterInt(cNum, nodeAddr, MN_P_NETERR_APP_OVERRUN,
                                     &stat);
    if (theErr == MN_OK) {
    ...
    theErr = coreBaseGetParameterInt(cNum, nodeAddr, MN_P_NETERR_APP_STRAY,
                                     &stat);
    if (theErr == MN_OK) {
    ...
    theErr = coreBaseGetParameterInt(cNum, nodeAddr, MN_P_NETERR_DIAG_CHKSUM,
                                     &stat);
    if (theErr == MN_OK) {
    ...
    theErr = coreBaseGetParameterInt(cNum, nodeAddr, MN_P_NETERR_DIAG_FRAG,
                                     &stat);
    if (theErr == MN_OK) {
    ...
    theErr = coreBaseGetParameterInt(cNum, nodeAddr, MN_P_NETERR_DIAG_OVERRUN,
                                     &stat);
    if (theErr == MN_OK) {
    ...
    theErr = coreBaseGetParameterInt(cNum, nodeAddr, MN_P_NETERR_DIAG_STRAY,
                                     &stat);
    if (theErr == MN_OK) {
    ...
    theErr = coreBaseGetParameterInt(cNum, nodeAddr, MN_P_NETERR_LOW_VOLTS,
                                     &stat);
    if (theErr == MN_OK) {
...
#if TRACE_BREAK
    _RPT3(_CRT_WARN,
          "%.1f in infcProbeForOffline(%d) calling infcResetNetRate,"
          "thread ID=" THREAD_RADIX_LONG "\n",
          infcCoreTime(), cNum, infcThreadID());
#endif
    //theErr = infcResetNetRate(cNum);
...
cnErrCode infcInitDataAcq(
    ...
    pNCS->DataAcq[NODE_ADDR(multiAddr)].SampRateMilliSec
        = SampleRateMicroSec * samplesPerDataAcq / 1000;
    pNCS->DataAcqMode[NODE_ADDR(multiAddr)] = theMode;
        ...
        if (knownNodeCount == 0) {
            ...
            if (netBreakLastAddrOK + 1 >= (int)knownNodeCount) {
                switch (pNCS->OurNetConnector) {
                    case MN_SRC_APP_NET:
                        errRet =
                            (cnErrCode)(MN_ERR_BRK_APP_00_HOST
                                        + (cnErrCode)(netBreakLastAddrOK));
                        break;
                    case MN_SRC_DIAG_NET:
                        errRet =
                            (cnErrCode)(MN_ERR_BRK_DIAG_00_HOST
                                        + (cnErrCode)(netBreakLastAddrOK));
                        break;
                    default:
                        errRet =
                            (cnErrCode)(MN_ERR_BRK_00_HOST
                                        + (cnErrCode)(netBreakLastAddrOK));
                        break;
                }
            }
                ...
                switch (pNCS->OurNetConnector) {
                    case MN_SRC_APP_NET:
                        errRet =
                            (cnErrCode)(MN_ERR_BRK_APP_00_01
                                        + (cnErrCode)(netBreakLastAddrOK));
                        break;
                    case MN_SRC_DIAG_NET:
                        errRet =
                            (cnErrCode)(MN_ERR_BRK_DIAG_00_01
                                        + (cnErrCode)(netBreakLastAddrOK));
                        break;
                    default:
                        errRet =
                            (cnErrCode)(MN_ERR_BRK_OTHER_00_01
                                        + (cnErrCode)(netBreakLastAddrOK));
                        break;
                }
            }
...
#if defined(__arm__)||defined(__aarch64__)
    infcGetDumpDir(winDir, sizeof(winDir) / sizeof(char));
#else
...
/**
    ...
    \return The pointer of the previous callback function that was assigned
    for this callback
...
/**
    ...
    \param[out] pResult Ptr to statistic that was collected.
    \return #cnErrCode; MN_OK if \p pResult was updated and \p pIsSet is
    non-zero if there was a stored statistic.
**/
//  SYNOPSIS:
...
/**
    ...
    \return #cnErrCode; MN_OK if dump was successfully created
**/
//  SYNOPSIS:
MN_EXPORT cnErrCode MN_DECL infcTraceDump(
    ...
    for (len = 0; len < MN_API_MAX_NODES; len++) {
        theHeader.nodeTypes[len] =
            SysInventory[cNum].InventoryNow.LastIDs[len];
    }
    ...
    if (theErr == MN_OK
        && !outStream.write((char *)&theNet.rxTraces[0],
    ...
    if (theErr == MN_OK
        && !outStream.write((char *)&theNet.txTraces[0],
...
int netPollerThread::Run(void *context) {
...
#if TRACE_POLL_THRD||TRACE_THREAD
    _RPT3(_CRT_WARN,
          "%.1f netPollerThread(%d): id=" THREAD_RADIX " starting\n",
          infcCoreTime(), cNum, UIthreadID());
#endif
    ...
    while (!Terminating()) {
        ...
        if (!m_halted) {
            ...
            if (SysInventory[cNum].OpenState == OPENED_ONLINE) {
                // To compensate for driver error in reacting to CTS line changes
                // This will enable global stop functionality on Linux
#if defined(__linux__)
                bool currentGlobalStopState = !pNCS->pSerialPort->GetCTS();
                if (!m_lastGlobalStopState && currentGlobalStopState) {
                    // Detected falling edge on CTS line => issue group shutdown
                    theNet.GroupShutdownRequest = true;
                }
                m_lastGlobalStopState = currentGlobalStopState;
#endif // End Linux driver compensation code
                ...
                if (theNet.GroupShutdownRequest) {
                    ...
                    for (size_t i = 0;
                        ...
                        if (sInfo.enabled) {
                            _RPT3(_CRT_WARN,
                                  "%.1f Group Shutdown(%d) type %0x\n",
                                  infcCoreTime(), theAddr,
                                  sInfo.theStopType.bits);
                            theErr = netNodeStop(theAddr, sInfo.theStopType,
                                                 false);
                        }
                    }
                    ...
                    // Check that the contents of the return packet match
                    // the contents of the sent packet
                    bool pktMismatch =
                        (outPkt.Byte.BufferSize != inPkt.Byte.BufferSize);
                    for (nodeulong iByte = 0;
                         iByte < outPkt.Byte.BufferSize && !pktMismatch;
                         iByte++) {
                        pktMismatch = pktMismatch 
                            || (outPkt.Byte.Buffer[iByte]
                                != inPkt.Byte.Buffer[iByte]);
                    }
...
#if TRACE_POLL_THRD||TRACE_THREAD
                        _RPT3(_CRT_WARN,
                              "%.1f netPollerThread(%d): probe cmd err=0x%x\n",
                              infcCoreTime(), cNum, theErr);
#endif
                        if (pktMismatch) {
...
#if TRACE_POLL_THRD||TRACE_THREAD
                        _RPT2(_CRT_WARN,
                              "%.1f netPollerThread(%d): Error Halted!\n",
                              infcCoreTime(), cNum);
#endif
                    }
...
#define ISC_MILESTONE_POLLER 0x5600
    ...
    if (SysInventory[pNCS->cNum].NodeInfo[0].theID.fld.devType == NODEID_CP) {
        errRet = netGetParameter(MULTI_ADDR(pNCS->cNum, 0), CP_P_FW_VERS,
                                 &fwVers);
    ...
    if (errRet == MN_OK 
        && SysInventory[pNCS->cNum].NodeInfo[0].theID.fld.devType == NODEID_MD) {
        errRet = netGetParameter(MULTI_ADDR(pNCS->cNum, 0), ISC_P_FW_VERSION,
                                 &fwVers);
    ...
    if (errRet == MN_OK 
        && (SysInventory[pNCS->cNum].NodeInfo[0].theID.fld.devType == NODEID_CS || 
            SysInventory[pNCS->cNum].NodeInfo[0].theID.fld.devType == NODEID_GS ||
            SysInventory[pNCS->cNum].NodeInfo[0].theID.fld.devType == NODEID_EP)) {
        errRet = netGetParameter(MULTI_ADDR(pNCS->cNum, 0), MN_P_FW_VERSION,
                                 &fwVers);
        if (errRet == MN_OK) {
            pollerSupported = true;
        }
...
MN_EXPORT cnErrCode MN_DECL infcBrakeControlSet(
    ...
    if (SysInventory[cNum].autoBrake[brakeNum].brakeMode == BRAKE_AUTOCONTROL
        ...
        attnOffErr =
            infcAutoBrakeSet(cNum, brakeNum, false,
...
MN_EXPORT cnErrCode MN_DECL infcAutoBrakeSet(
    ...
    if (enabled && cNum != NET_NUM(theMultiAddr) && theMultiAddr != MN_UNSET_ADDR) {
        return MN_ERR_BADARG;
    }
...
#if TRACE_BRAKE
    _RPT4(_CRT_WARN,
          "infcAutoBrakeSet: cNum=%d, brake=%d, enable=%d, addr=%d\n",
    ...
    if (theMultiAddr != MN_UNSET_ADDR) {
        ...
        // Turning on auto-brake for a node that does not exist. Set state off
        theErr = infcBrakeSet(cNum, brakeNum, FALSE);
    }
...
mnNetInvRecords::mnNetInvRecords() {
    ...
    sendSerNum = 0;                         // Start counting from 0
    respSerNum = 0;
    txTraceIndx = 0;
    rxTraceIndx = 0;
    txTraces = NULL;
...
void mnNetInvRecords::setupBrakes() {
    ...
    if (AccessInfo[0].Fld.NetSource != MN_SRC_APP_NET) {
        return;
    }
    for (size_t i = 0; i < MAX_BRAKES; i++) {
        // Run ignoring success/failure
        infcAutoBrakeSet(pNCS->cNum, i, autoBrake[i].enabled,
                         autoBrake[i].relatedNode);
    }
}
//                                                                             *
//******************************************************************************
...
char *_mnPowerReg::StateStr(char *buffer, size_t size) {
    ...
    lastIndx = fld.AcPhaseActiveLast >= PWR_CNT 
               ? PWR_CNT - 1 : fld.AcPhaseActiveLast;
    accumIndex = fld.AcPhaseActiveAccum >= PWR_CNT
                 ? PWR_CNT - 1 : fld.AcPhaseActiveAccum;

    // Format up a nicely formatted string
    snprintf(buffer, size,
             "AC Phases Now: %s%s, Last AC Phases: %s%s, "
             "AC Phases since reset: %s\n"
             "Bus loss: %s; Phase loss: %s\n"
             "Under Operating Voltage: %s; logic supply detected: %s\n",
             pwrTypes[nowIndx], fld.AcPhaseDir ? "(reverse)" : "",
             pwrTypes[lastIndx], fld.AcPhaseDirLast ? "(reverse)" : "",
             pwrTypes[accumIndex], fld.InBusLoss ? "yes" : "no",
             fld.InWiringError ? "yes" : "no",
             fld.InUnderOperV ? "yes" : "no",
             fld.LogicPwrSupply ? "yes" : "no");
